...
// ✅ FIXED IMPORT: aiService is now a default import, types are named imports
import aiService, { type AIBriefing, type UserStats } from '../../lib/aiService';

interface AIBriefingWidgetProps {
  user: any;
  tasks: any[];
  // Make stats optional in props to prevent crash if parent passes undefined
  stats?: UserStats; 
}

const AIBriefingWidget: React.FC<AIBriefingWidgetProps> = ({ 
  user, 
  tasks, 
  // 🛡️ DEFAULT VALUE: Prevents "undefined" crash
  ...
  const fetchBriefing = useCallback(async (force = false) => {
    // 🛡️ DOUBLE CHECK: Ensure we have an ID and a Name
    ...
    try {
      // Normalize name (handle Supabase Auth vs Database Profile structure)
      ...
      const data = await aiService.generateDailyBriefing(
        ...
        userName,
        pendingTasks,
        stats // Safe because we set a default value above
      );
      ...
      setBriefing({
        ...
        message: "Unable to sync with sovereign core.",
        color: "text-gray-400"
      });
    } finally {
      setIsLoading(false);
  ...
  useEffect(() => {
    // 🛡️ WAIT FOR DATA: Only run if we actually have a valid User ID
      ...
      <div className="relative p-8 h-full flex flex-col z-10">
        ...
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-[10px] font-black text-white/40 uppercase tracking-[0.3em]">Margdarshak AI</h3>
        ...
        <AnimatePresence mode="wait">
          {isLoading ? (
            <motion.div key="loader" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex-1 flex flex-col items-center justify-center">
              ...
              <p className="text-[10px] text-white/20 mt-2 uppercase tracking-widest">Neural Handshake...</p>
            </motion.div>
          ) : (
