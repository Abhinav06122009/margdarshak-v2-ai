...
type Mode = 'deepsearch' | 'quickchat';

// 🛡️ SECURITY: Valid Tier IDs for Premium + AI (Sovereign Mode)
// ✅ FIXED: Added 'premium+ai' to match your database value exactly
const SOVEREIGN_TIERS = ['premium_ai', 'extra_plus', 'premium_plus', 'premium+ai'];
...
const AIPage = () => {
  ...
  const [mode, setMode] = useState<Mode>('deepsearch');
  ...
  const checkSubscription = async () => {
    ...
    if (user) {
      ...
      if (data?.subscription_tier) {
        // console.log("Current Tier:", data.subscription_tier); // Debugging
  ...
  const isSovereignUser = () => SOVEREIGN_TIERS.includes(subscriptionTier);

  // ✅ MAIN SEND FUNCTION
  const executeSend = async (textToSend: string, modeToUse: Mode, imageToSend: File | null = null) => {
    // 🔒 LOGIC: IF NOT Premium+AI, REQUIRE KEY
    if (!isSovereignUser() && !userApiKey) {
    ...
    try {
      const WORKER_URL = "https://margdarshak-ai.abhinav-vsavwe4899.workers.dev"; 
      const { data: { session } } = await supabase.auth.getSession();
      
      const headers: any = {
        // 🧠 LOGIC: 
        // 1. If Sovereign (Premium+AI) -> Do NOT send X-User-API-Key (Backend uses System Key)
        // 2. If Free/Premium -> MUST send X-User-API-Key
        ...(!isSovereignUser() && userApiKey ? { "X-User-API-Key": userApiKey } : {}),
  ...
  const exportToPDF = (c: string, t: string) => {
    ...
    doc.setFontSize(18);
    doc.text("MARGDARSHAK OMEGA NOTES", margin, 20);
    doc.setFontSize(10);
        ...
        <DialogContent className="bg-[#121212] border border-amber-500/20 text-white sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription className="text-gray-400 pt-2">
              Free and Standard Premium users must provide a <strong>SambaNova Cloud API Key</strong>. <br/>
              <span className="text-emerald-400 text-xs mt-2 block">
                Upgrade to <strong>Premium + AI</strong> to use our sovereign neural engine without a key.
              </span>
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              ...
              <p className="text-[10px] text-gray-500">
                Don't have one? <a href="https://cloud.sambanova.ai/" target="_blank" className="text-amber-500 hover:underline">Get a free key here</a>.
              </p>
            </div>
            <div className="flex items-center justify-between border-t border-white/10 pt-4 mt-2">
               <span className="text-xs text-gray-400">Want hassle-free AI?</span>
               <Link to="/upgrade" className="text-xs font-bold text-emerald-400 hover:underline">Get Premium + AI →</Link>
            </div>
          </div>
          <DialogFooter>
        ...
        <DialogContent className="bg-gradient-to-br from-[#1a0b2e] to-black border border-purple-500/30 text-white sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-xl font-bold text-purple-400">
              <Crown className="w-5 h-5 fill-current" /> Premium + AI Exclusive
            </DialogTitle>
            <DialogDescription className="text-gray-300 pt-2">
              This specific AI model or feature is reserved for Premium + AI users.
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
             <div className="p-4 bg-purple-500/10 rounded-xl border border-purple-500/20 text-sm text-purple-200">
                🚀 Upgrade to Premium + AI to unlock Deep Web Search, Image Analysis, and Flux-1 Generation.
             </div>
          </div>
          <DialogFooter>
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-black italic text-transparent bg-clip-text bg-gradient-to-r from-white to-slate-400">
                MARGDARSHAK <span className="text-amber-500">OMEGA</span>
              </h1>
              <div className="flex items-center gap-2 text-[9px] font-bold text-amber-500/80 uppercase tracking-[0.2em]">
                {isSovereignUser() && <span className="text-purple-400 flex items-center gap-1"><Zap size={10} /> Premium + AI Active</span>}
                {!isSovereignUser() && subscriptionTier === 'premium' && <span className="text-blue-400 flex items-center gap-1"><Sparkles size={10} /> Premium (BYOK)</span>}
                {!isSovereignUser() && subscriptionTier !== 'premium' && <span className="text-gray-400 flex items-center gap-1"><Lock size={10} /> Free Plan (BYOK)</span>}
              </div>
            </div>
          </div>

          <div className="flex items-center gap-2 mt-3 md:mt-0">
             {/* Key Button: Visible for EVERYONE who isn't Premium+AI */}
             {!isSovereignUser() && (
             ...
             <div className="flex bg-black/40 p-1 rounded-full border border-white/5">
                {[
                  { id: 'deepsearch', icon: Globe, label: 'Deep' },
          ...
          <div className="space-y-8 pb-10">
            {messages.length === 0 && (
              <div className="h-[50vh] flex flex-col items-center justify-center opacity-30 select-none">
                ...
                <p className="font-bold tracking-[0.5em] text-xs uppercase">Awaiting Sovereign Query</p>
                {!isSovereignUser() && !userApiKey && (
