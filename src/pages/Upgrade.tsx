...
const Upgrade = () => {
  ...
  const updateSubscription = async (tier: 'premium' | 'premium_ai') => {
    try {
      ...
      toast({
        ...
        description: `Welcome to ${tier === 'premium_ai' ? 'Premium + AI' : 'Premium'}.`,
  ...
  useEffect(() => {
    ...
    // Listener for Uropay Success Event
    // Note: Verify the exact event name in Uropay docs. Common patterns used below.
    const handleMessage = (event: MessageEvent) => {
      // Check origin if possible for security
      // if (event.origin !== "https://uropay.me") return; 

      if (event.data === 'payment_success' || event.data?.status === 'success') {
        // Determine tier based on amount or logic. 
        // For now, if we detect a success on this page, we might need to ask user or 
        // verify the amount. Since Uropay events might be generic:
        
        // FAILSAFE: You might need to check transaction ID with backend.
        // For this implementation, we assume the user clicked the specific button.
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 py-8 md:py-16">
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-amber-500/10 border border-amber-500/20 text-amber-500 text-xs font-black uppercase tracking-widest mb-4">
            <Crown size={14} /> Sovereign Intelligence
          </div>
          ...
          <p className="text-lg text-gray-400 leading-relaxed max-w-2xl mx-auto">
            Unlock the full potential of Margdarshak Omega. Generate complex scientific visuals, store unlimited knowledge, and access the most powerful AI models.
          </p>
        </motion.div>
        ...
        >
          ...
          <motion.div variants={itemVariants} className="relative p-8 rounded-[2.5rem] bg-[#0A0A0A] border border-white/5">
            ...
            <div className="mt-8 space-y-4">
               ...
               {['Sovereign AI Model', 'Deep Search', 'Priority Support'].map((feat, i) => (
          ...
          >
            ...
            {/* ⚠️ ADDED ONCLICK TO SIMULATE SUCCESS FOR TESTING IF UROPAY FAILS */}
            <div onClick={() => {
              // Dev-only: If Uropay callback fails, verify manually here
              ...
              <a 
                ...
                // IMPORTANT: If Uropay supports redirects, add your success URL here in the dashboard
                // e.g., https://your-site.com/payment-success?tier=premium
              >
                Get Premium
              </a>
            </div>
            ...
            <div className="mt-8 space-y-4">
              {[
               ...
               <div className="flex items-center gap-3 text-sm">
                  ...
                  <span className="text-gray-300 font-medium">AI Chat (Bring Your Own Key)</span>
               </div>
               <div className="flex items-center gap-3 text-sm text-gray-600">
                  <X size={16} className="shrink-0" />
                  Sovereign AI (No Key Needed)
               </div>
            </div>
          </motion.div>
          ...
          >
            ...
            <h3 className="text-xl font-bold text-purple-400 mb-2 flex items-center gap-2">
              Premium + AI <BrainCircuit size={16} className="fill-current" />
            </h3>
            ...
            <p className="text-sm text-gray-400 mb-8 h-10">The ultimate power-suite with Sovereign Intelligence.</p>
            
            {/* ⚠️ DEV MODE: Clicking wrapper triggers update for testing */}
            {/* In production, remove onClick and rely on webhook/redirect */}
            <div onClick={() => {
               // UNCOMMENT BELOW FOR MANUAL OVERRIDE IF UROPAY FAILS
               // if (confirm("Simulate Payment Success for Premium+AI?")) updateSubscription('premium_ai');
            }}>
              <a 
                href="#" 
            ...
            <div className="mt-8 space-y-4">
              <div className="flex items-center gap-3 text-sm">
                  ...
                  <span className="text-white font-bold text-emerald-400">Sovereign AI (No Key Req)</span>
              </div>
              {[
                'Deep Web Search & Vision',
                'All Premium Features',
                '500 GB Cloud Storage',
                'Predictive Grade Analytics',
                'Automatic Timetable Gen',
                'Dedicated 24/7 AI Coach'
              ].map((feature, i) => (
                <div key={i} className="flex items-center gap-3 text-sm">
                  <div className="p-0.5 rounded-full bg-purple-500/20 text-purple-400">
                    <Check size={12} strokeWidth={4} />
        ...
        <div className="mt-24 max-w-3xl mx-auto">
          ...
          <div className="space-y-4">
             <FaqItem q="Is this a one-time payment?" a="Yes, both Premium and Premium + AI are lifetime access plans. No recurring monthly fees." />
             <FaqItem q="What is Sovereign AI?" a="Sovereign AI means you use our inbuilt high-performance neural engine without needing your own API key. It's fully managed and unrestricted." />
             <FaqItem q="Can I upgrade from Premium to Premium + AI?" a="Yes, you can upgrade at any time by paying the difference. Contact support for assistance." />
        ...
        <footer className="mt-20 pt-10 border-t border-white/5 text-center">
            <p className="text-xs text-gray-600 font-medium">
              Secure Payments via Uropay • © 2025 Margdarshak Omega
            </p>
        </footer>
